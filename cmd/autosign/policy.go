...
type PolicyConfig struct {
	// ReservedCertnames are names the operator issues to itself. The CA auth.conf
	// grants admin rights by certname as well as by extension, so a name on this
	// list must never be handed to an agent: whoever holds a certificate under it
	// is a CA admin. Enforced before any policy is consulted, so no policy -
	// including any: true - can hand one out.
	ReservedCertnames []string `yaml:"reservedCertnames,omitempty"`
...
func evaluatePolicies(cfg *PolicyConfig, certname string, csr *x509.CertificateRequest) bool {
	// Both checks precede every policy, so any: true cannot waive them.
	if isReservedCertname(cfg, certname) {
		return false
	}
	if !subjectMatchesCertname(certname, csr) {
		return false
	}
...
// isReservedCertname reports whether the requested name belongs to the
// operator. Compared case-insensitively because Puppet lowercases certnames.
func isReservedCertname(cfg *PolicyConfig, certname string) bool {
	for _, r := range cfg.ReservedCertnames {
		if strings.EqualFold(r, certname) {
			return true
		}
	}
	return false
...
// subjectMatchesCertname reports whether the CSR asks for the same name the CA
// is about to issue under.
//
// puppetserver takes the certname from the request path and passes it as the
// argument, while the CN lives in the CSR subject. The policy decides about the
// argument, so a CSR whose subject says something else would be judged under a
// name it does not carry. Rather than rely on puppetserver rejecting that
// first, the mismatch is refused here.
func subjectMatchesCertname(certname string, csr *x509.CertificateRequest) bool {
	return strings.EqualFold(csr.Subject.CommonName, certname)
}
