...
func classify(cfg *ENCConfig, certname string) (string, error) {
	url := cfg.URL + strings.ReplaceAll(cfg.Path, "{certname}", certname)
	...
	if resp.StatusCode == http.StatusNotFound {
		return "", &notFoundError{msg: fmt.Sprintf("HTTP 404: %s", url)}
	}
...
func loadFacts(certname string) map[string]interface{} {
	factsPath := filepath.Join("/opt/puppetlabs/server/data/puppetserver/yaml/facts", certname+".yaml")
...
func saveCache(dir, certname, data string) error {
	...
	path := filepath.Join(dir, certname+".yaml")
