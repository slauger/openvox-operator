...
Server pods use a read-only root filesystem by default for security hardening. This is controlled by the `.spec.readOnlyRootFilesystem` field in the Config CRD (default: `true`). All writable paths are backed by emptyDir volumes and `server-var-dir` is redirected to `/run/puppetserver`. All configuration comes from ConfigMaps and Secrets. This section documents every volume mounted into Server and CA pods.
